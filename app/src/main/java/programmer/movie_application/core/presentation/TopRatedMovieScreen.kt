...
fun TopRatedMovieScreen(
    ...
    if(movieStateList.topRateMovieList.isEmpty()){
        Box(
        ...
        LazyVerticalGrid(
            ...
            items(movieStateList.topRateMovieList.size){index ->
                MovieCard(
                    movie = movieStateList.topRateMovieList[index],
                    navController = navController)
                Spacer(modifier = Modifier.height(16.dp))
                if(index >= movieStateList.topRateMovieList.size - 1 && !movieStateList.isLoading){
                    onEvent(MovieListUiEvents.Paginate(Category.TOP_RATED))
                }
            }
        }
    }
