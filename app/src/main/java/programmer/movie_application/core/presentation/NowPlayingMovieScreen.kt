...
fun NowPlayingMovieScreen(
    ...
    if(movieStateList.nowPlayingMovieList.isEmpty()){
        Box(
        ...
        LazyVerticalGrid(
            ...
            items(movieStateList.nowPlayingMovieList.size){index ->
                MovieCard(
                    movie = movieStateList.nowPlayingMovieList[index],
                    navController = navController)
                Spacer(modifier = Modifier.height(16.dp))
                if(index >= movieStateList.nowPlayingMovieList.size - 1 && !movieStateList.isLoading){
                    onEvent(MovieListUiEvents.Paginate(Category.NOW_PLAYING))
                }
            }
        }
    }
