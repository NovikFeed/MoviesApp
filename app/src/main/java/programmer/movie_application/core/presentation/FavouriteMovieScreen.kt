...
fun FavouriteMovieScreen(
    ...
    if(movieStateList.favouriteMovieList.isEmpty()){
        Box(
        ...
        LazyVerticalGrid(
            ...
            items(movieStateList.favouriteMovieList.size){index ->
                MovieCard(
                    movie = movieStateList.favouriteMovieList[index],
                    navController = navController)
                Spacer(modifier = Modifier.height(16.dp))
                if(index >= movieStateList.favouriteMovieList.size - 1 && !movieStateList.isLoading){
                    onEvent(MovieListUiEvents.Paginate(Category.FAVOURITE))
                }
            }
